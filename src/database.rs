...
mod tests {
    ...
    #[tokio::test]
    async fn test_sync_persistence_mode() {
        // Test that sync mode waits for persistence
        let tmp_dir = TempDir::new().unwrap();
        let db_path = tmp_dir.path().join("test.db");
        let keys = Keys::generate();
        let task_tracker = TaskTracker::new();
        ...
        database
            .save_store_command(
                StoreCommand::SaveSignedEvent(Box::new(event.clone()), Scope::Default),
                None,
            )
            .await
        ...
        // Save should complete relatively quickly
        assert!(
            duration.as_millis() < 100,
            "Sync save took too long: {:?}",
            duration
        );
    ...
    #[tokio::test]
    async fn test_async_persistence_mode() {
        // Test that async mode returns immediately
        let tmp_dir = TempDir::new().unwrap();
        let db_path = tmp_dir.path().join("test.db");
        let keys = Keys::generate();
        let task_tracker = TaskTracker::new();
        ...
        // Async save should be nearly instant
        assert!(
            duration.as_micros() < 5000,
            "Async save took too long: {:?}",
            duration
        );
    }
